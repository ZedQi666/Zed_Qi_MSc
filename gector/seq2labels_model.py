...
class Seq2Labels(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self.enc_bi_gru_correct = torch.nn.GRU(input_size=text_field_embedder._token_embedders['bert'].get_output_dim(),
                                       hidden_size=int(text_field_embedder._token_embedders['bert'].get_output_dim()/2), dropout=0.2, bidirectional=True)
        ...
        # Detection Network
        self.enc_bi_gru = torch.nn.GRU(input_size=text_field_embedder._token_embedders['bert'].get_output_dim(), hidden_size=256, dropout=0.2, bidirectional=True)

        self.tag_detect_projection_layer = TimeDistributed(

            Linear(512, self.num_detect_classes))
    ...
    def forward(self,  # type: ignore
        ...
        #Correct Network
        h_0_correct = torch.zeros(2, encoded_text.shape[1], int(encoded_text.shape[2]/2))
        bi_gru_correct = self.enc_bi_gru_correct(encoded_text, h_0_correct)[0]
        logits_labels = self.tag_labels_projection_layer(bi_gru_correct)

        # Detection_Network
        h_0 = torch.zeros(2, encoded_text.shape[1], 256)
        bi_gru_final_hidden_layer = self.enc_bi_gru(encoded_text, h_0)[0]
        logits_d = self.tag_detect_projection_layer(bi_gru_final_hidden_layer)
